...
const LOGIN_URL = 'https://www.felmat.net/advertiser/login';
const CONVERSION_CONFIRM_URL = 'https://www.felmat.net/advertiser/conversion/confirm';
...
async function login(page: Page, loginId: string, password: string): Promise<void> {
  ...
  await page.fill('input[name="c_username"]', loginId);
...
async function downloadCsvForProgram(
  ...
  await page.goto(CONVERSION_CONFIRM_URL, { waitUntil: 'networkidle', timeout: DEFAULT_TIMEOUT });
  await page.waitForTimeout(1000);
  await saveScreenshot(kvStore, page, screenshotKeys, `confirm-page-${slug}`);

  // プログラム選択（jQuery Chosen プラグイン対応）
  ...
  await clickFirst(page, [
    ...
    'button[type="submit"]:has-text("一覧を抽出")',
    'button:has-text("上記条件で一覧を抽出")',
  ], '検索ボタン');
  ...
  await clickFirst(page, [
    ...
    'button.dropdown-toggle:has-text("CSVダウンロード")',
  ], 'CSVダウンロードドロップダウン');

  await page.waitForTimeout(500);

  // 「Excel表示用」ボタンをクリックしてCSVダウンロード
  const [download] = await Promise.all([
    page.waitForEvent('download', { timeout: DOWNLOAD_TIMEOUT }),
    clickFirst(page, [
      'button[name="csv_dl"][value="csv_dl_ex"]',
      'button:has-text("Excel表示用")',
    ], 'Excel表示用CSVダウンロード'),
  ]);
  ...
  const kvStoreKey = `csv_felmat_${slug}_${timestamp}`;

  await kvStore.setValue(kvStoreKey, content, { contentType: 'text/csv; charset=utf-8' });

  const suggested = download.suggestedFilename() || `felmat_${slug}_${timestamp}.csv`;
...
async function selectChosenOption(page: Page, value: string, label: string): Promise<void> {
  log.info(`プログラム選択: ${label} (value=${value})`);

  // 方法1: 裏のselect要素に直接値をセットし、Chosen UIを更新
  const set = await page.evaluate((val) => {
    // Chosenの命名規則: コンテナIDが {selectId}_chosen
    // sel_adv_id_chosen → select#sel_adv_id
    const selectors = ['#sel_adv_id', 'select[name="adv_id"]', 'select[name="sel_adv_id"]'];
    for (const sel of selectors) {
      const select = document.querySelector(sel) as HTMLSelectElement | null;
      if (select) {
        select.value = val;
        select.dispatchEvent(new Event('change', { bubbles: true }));
        // jQuery Chosen の更新トリガー
        try {
  ...
  if (set) {
    log.info('プログラム選択完了（evaluate経由）');
    return;
  }

  // 方法2: Chosen UIをクリックして操作
  log.info('evaluate失敗、Chosen UIで選択を試行');
  const chosenContainer = page.locator('#sel_adv_id_chosen, .chosen-container').first();
  ...
  log.info('プログラム選択完了（Chosen UI経由）');
}
...
async function saveScreenshot(
  ...
  const key = `screenshot_felmat_${label}_${Date.now()}`;
